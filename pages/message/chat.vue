<template>
	<view>
		<cola
			...
			:inputDisabled="inputDisabled || this.order.solved"
			@sendMsg="send"
			@moreClick="clickMore"
		></cola>
		<u-popup :show="showPopup">
			<view class="flex flex-direction align-center padding">
				<u-rate :count="starcount" size="30" activeColor="#f3f300" v-model="ratevalue"></u-rate>
				<view class="flex margin">
					<button plain="true" class="margin-right" @click="this.showPopup=false">取消</button>
					<button plain="true" type="primary" @click="toevaluate">确认</button>
				</view>
			</view>
		</u-popup>
	</view>
...
<script>
	...
	import confirm_solved from "@/utils/remote_data_service/confirm_solved.js"
	import evaluate from "@/utils/remote_data_service/evaluate.js"
	export default {
		data() {
			return {
				openid: '',
		...
		methods: {
			send(e) {
				...
				var message = {
					...
					message: e,
					img: '',
					avator: this.$store.state.login.avatar
				}
				var id = this.$store.state.orders.idToBeSearch
				add_message(message, id, this.openid, this.order.doctorinfo.openid, 'user')
				.then(res => {
					console.log(res);
				})
				.catch(res => {
			...
			clickMore(e) {
				var _this = this
				if(e===0){
					if(this.order.solved || this.inputDisabled){
						uni.showToast({
							title:"问诊已经结束",
							icon:"none"
						})
						...
						// tempFilePath可以作为 img 标签的 src 属性显示图片
							...
							.then(res => {
								var message = {
									...
									message: '',
									img: pictureid,
									avator: _this.$store.state.login.avatar
								}
								var id = _this.$store.state.orders.idToBeSearch
								return add_message(message, id, _this.openid, _this.order.doctorinfo.openid, 'user')
							})
							.then(res => {
								console.log(res);
							})
					...
					uni.showModal({
						title:'即将完成订单',
						content:"点击确认完成之后，将不能继续咨询医生，您确认要完成订单吗？",
						showCancel: true,
						success(res) {
							if (res.confirm) {
								var id = _this.$store.state.orders.idToBeSearch
					...
					if(this.order.point !== -1 || this.hasEvalued){
						uni.showToast({
							title:"问诊已评价",
							icon:"none"
						})
			...
			toevaluate(){
				...
				var id = this.$store.state.orders.idToBeSearch
				evaluate(id, this.ratevalue)
				.then(res => {
					this.hasEvalued = true
					uni.showToast({
						icon:"success",
						title:res
					})
